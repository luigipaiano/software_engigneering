...
public class Commento2RestControllerTest {
	...
	private void setUp() {
        ...
        commento.setData((Timestamp) new Date());
        commento.setTipo(4);
        commento.setVal(4);
        commento.setDescrizione("descrizione_prova");
    
        
    	PropostaDTO proposta_prova = new PropostaDTO();
		proposta_prova.setDatacreamodpr(new Date());
		
		BeneservizioDTO servizio_prova = new BeneservizioDTO();
		
		servizio_prova.setCategoria("appartamenti");
		proposta_prova.setBeneservizio(servizio_prova);
